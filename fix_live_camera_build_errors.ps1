Set-Location C:\Projects\pop-bus-web

Write-Host "[1/3] Fixing LIVE_CAMERA and unused functions..."
...
$nodeScript | Set-Content -Encoding UTF8 .\fix_live_camera_build_errors.js

node .\fix_live_camera_build_errors.js

Write-Host "[2/3] Running TypeScript/Vite build..."
npm run build

if ($LASTEXITCODE -eq 0) {
  Write-Host "[3/3] DONE. Build passed. Now run: npm run dev"
} else {
  Write-Host "[3/3] Build still failed. Send the new terminal error."
  exit $LASTEXITCODE
}
